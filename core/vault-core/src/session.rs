...
/// then lay a fresh FAT filesystem in the decrypted data area so it opens as
/// an empty volume rather than "unknown filesystem" (doc §7). `dev` must
/// already be `params.size` bytes.
pub fn create_container(
    ...
    // Re-derive the master keys through the header we just wrote so we can
    // open the decrypted data area and format it.
    ...
    vc_fs::format_fat(Box::new(decrypted))
}
...
impl Session {
    ...
    pub fn unlock_outer_protected(
        ...
        let mut dev = FileDevice::open_rw(path)?;
        ...
        let decrypted = DecryptedDevice::new(Box::new(dev), xts, &outer_geo);
