...
fn main() -> ExitCode {
    ...
    match cli.command {
        Command::Create {
            ...
            pim,
            keyfiles,
        } => cmd_create(container, size, encryption, hash, pim, keyfiles),
        Command::Info {
            container,
            pim,
            keyfiles,
...
fn cmd_create(
    ...
    if let Err(e) = std::fs::write(&container, vec![0u8; size as usize]) {
    ...
    match vault_core::create_container(Box::new(dev), &params) {
        Ok(()) => {
            eprintln!("created {encryption} / {hash} container with an empty FAT filesystem");
            ExitCode::SUCCESS
        }
        Err(e) => {
            eprintln!("error: {e}");
