...
pub use create::{create_volume, CreateParams};
...
pub fn find_header_at(
    ...
    let total = positions.len() * vc_types::registry::PRFS.len();
    ...
    for &position in positions {
        ...
        // Fast path: the overwhelming default (first PRF in popularity
        // order) alone, so a stock SHA-512 container never pays for the
        // other derivations at all.
        let prfs = vc_types::registry::PRFS;
        progress(step, total, prfs[0].name);
        step += 1;
        let key0 = derive_for(&prfs[0], secret, &salt, max_key_len)?;
        match try_schemes(&supported, &key0, &enc, position) {
        ...
        // Remaining PRFs derive in parallel across cores (doc §6): the
        // wrong-password worst case costs max(PRF) wall time, not sum(PRF).
        let rest: Vec<&'static vc_types::Prf> = prfs.iter().skip(1).collect();
        let keys: Vec<VcResult<vc_crypto::HeaderKey>> = std::thread::scope(|s| {
            let handles: Vec<_> = rest
                .iter()
                .map(|prf| s.spawn(|| derive_for(prf, secret, &salt, max_key_len)))
                .collect();
            handles.into_iter().map(|h| h.join().unwrap()).collect()
        });
        for (prf, key) in rest.into_iter().zip(keys) {
            progress(step, total, prf.name);
            step += 1;
            match try_schemes(&supported, &key?, &enc, position) {
                SchemeOutcome::Found(header, scheme) => {
                    return Ok(FoundHeader {
                        header: *header,
                        scheme,
